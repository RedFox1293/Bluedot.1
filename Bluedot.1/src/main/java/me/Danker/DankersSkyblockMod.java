...
public class DankersSkyblockMod {
    ...
    private final World world = mc.theWorld;
    ...
    public void init(FMLInitializationEvent event) {
        ...
        keyBindings[6] = new KeyBinding("Mage Weapon Bind", Keyboard.KEY_I, "Danker's Skyblock Mod");
        keyBindings[7] = new KeyBinding("Ghost Block Bind", Keyboard.KEY_G, "Danker's Skyblock Mod");
        keyBindings[8] = new KeyBinding("Rogue Sword Bind", Keyboard.KEY_N, "Danker's Skyblock Mod");
        keyBindings[9] = new KeyBinding("AOTS Valk Bind", Keyboard.KEY_K, "Danker's Skyblock Mod");
        keyBindings[10] = new KeyBinding("Giant's Sword Dae Axe Swap Bind", Keyboard.KEY_L, "Danker's Skyblock Mod");
        keyBindings[11] = new KeyBinding("Jerry-Chine Gun Bind", Keyboard.KEY_M, "Danker's Skyblock Mod");
        keyBindings[12] = new KeyBinding("Katana Macro", Keyboard.KEY_C, "Danker's Skyblock Mod");
        keyBindings[13] = new KeyBinding("Soul Whip", Keyboard.KEY_O, "Danker's Skyblock Mod");
    ...
    public void onTick(TickEvent.ClientTickEvent event) throws InterruptedException {
        ...
        if (keyBindings[9].isPressed()) {
            ...
            for (int i = 0; i <= 8; i++) {
                ...
                if ((item != null && item.getDisplayName().contains("Valkyrie"))) {
                    this.sword = i;
                    Thread.sleep(1500);
                }
            }
        ...
        if (keyBindings[11].isKeyDown())
            for (int i = 0; i <= 8; i++) {
                ItemStack item = player.inventory.getStackInSlot(i);
                if ((item != null && item.getDisplayName().contains("Jerry-chine Gun"))) {
                    player.inventory.currentItem = i;
                    mc.playerController.sendUseItem(mc.thePlayer, world, player.inventory.getStackInSlot(i));
                            break;
                        }
                ...
                if (keyBindings[13].isKeyDown())
                    for (int i = 0; i <= 8; i++) {
                        ItemStack item = player.inventory.getStackInSlot(i);
                        if ((item != null && item.getDisplayName().contains("Soul Whip"))) {
                            player.inventory.currentItem = i;
                            mc.playerController.sendUseItem(mc.thePlayer, world, player.inventory.getStackInSlot(i));
                            break;
                        }
                ...
                if (keyBindings[12].isKeyDown())
                    for (int i = 0; i <= 8; i++) {
                        ItemStack item = player.inventory.getStackInSlot(i);
                        if ((item != null && item.getDisplayName().contains("Katana"))) {
                            player.inventory.currentItem = i;
                            mc.playerController.sendUseItem(mc.thePlayer, world, player.inventory.getStackInSlot(i));
                            Thread.sleep(4000);
                            break;
                    ...
                    for (String s : scoreboard) {
                        ...
                        if (sCleaned.contains("Sven Packmaster")) {
                            ...
                            if (sCleaned.contains("F1")) {
                                ...
                                DisplayCommand.display = "catacom4rbs_floor_three";
                            } else if (sCleaned.contains("F4")) {
        ...
        public void onKey (KeyInputEvent event){
            ...
            if (keyBindings[5].isPressed()) {
                for (int i = 0; i <= SimonCommand.simonAmount; i++) {
                    try {
                        Method method = mc.getClass().getDeclaredMethod("func_147121_ag");
